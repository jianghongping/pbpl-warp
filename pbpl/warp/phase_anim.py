...
def get_parser():
    parser = argparse.ArgumentParser(
        description='Make animation from HDF5 dump')
    parser.add_argument(
        'conf', metavar='CONF',
        help='Configuration file (e.g., phase-anim.ini)')
    return parser
...
def get_args():
    ...
    c.read(args.conf)
    args.input_path = c.get('input', 'path')
    args.input_i0 = c.getint('input', 'i0')
    args.input_i1 = c.getint('input', 'i1')
    args.input_istep = c.getint('input', 'istep')
    args.input_num_particles = c.getint('input', 'num_particles')

    args.output_filename = c.get('output', 'filename')

    args.xaxis_value = c.get('xaxis', 'value')
    args.xaxis_title = c.get('xaxis', 'title')
    args.xaxis_min = c.getfloat('xaxis', 'min')
    args.xaxis_max = c.getfloat('xaxis', 'max')

    args.yaxis_value = c.get('yaxis', 'value')
    args.yaxis_title = c.get('yaxis', 'title')
    args.yaxis_min = c.getfloat('yaxis', 'min')
    args.yaxis_max = c.getfloat('yaxis', 'max')
